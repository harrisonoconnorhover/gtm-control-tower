...
export const demoStages: DemoStage[] = [
  {
    id: 'ingest',
    label: 'Ingest',
    system: 'n8n + CRM state',
    detail: 'Reset a named synthetic CRM batch without trusting its shape.',
    result: '10 raw contacts received',
  },
  {
    id: 'enrich',
    label: 'Enrich',
  ...
  },
  {
    ...
    result: '3 unsafe writes contained',
  },
  {
    id: 'model',
    label: 'Model',
...
export const scenarios: Record<ScenarioKey, Incident> = {
  'duplicate-surge': {
    ...
    detail: 'Two duplicate rows collide into two canonical contact identities.',
    recommendation: 'Merge each duplicate into its canonical contact while retaining the source rows and receipt.',
    severity: 'critical',
  },
  'routing-overload': {
    id: 'routing-overload',
    ...
    detail: 'Five active Northeast enterprise contacts share an overloaded route.',
    recommendation: 'Shift those active contacts to the Central enterprise overflow owner.',
    severity: 'warning',
  },
  'stage-regression': {
    id: 'stage-regression',
    ...
    detail: 'One active contact remains below its expected lifecycle stage.',
    recommendation: 'Restore the expected stage and record the lifecycle replay as an executed repair.',
    severity: 'critical',
  },
};
